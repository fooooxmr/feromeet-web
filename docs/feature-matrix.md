...
| Invitation | Expense split (`I_PAY` / `SPLIT` / `YOU_PAY`), date format chips, optional comment; `price` is always 0 | Invite sheet without a budget field |
...
| Chat | History, realtime messages, status, typing and online state | History always; realtime only with `EXPO_PUBLIC_WS_URL` |
| Profile | Photos, about text, height, tags, location and readiness | Editable profile sections |
| Safety | Report by email (`mailto:hello@inera.by`), logout and irreversible account deletion | Settings and confirmation dialogs |
...
1. One user sends an invitation with an expense split and a date format.
...
- Real SMS delivery and live payload envelopes.
